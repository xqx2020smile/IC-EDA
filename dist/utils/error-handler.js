...
export class ErrorHandler {
     ...
     */
    static toMcpError(error, toolName) {
        ...
        if (error.code === 'ENOENT') {
            return new McpError(ErrorCode.MethodNotFound, `Verible tool '${toolName}' not found. Please ensure Verible is installed.`);
        }
        if (error.code === 'ETIMEDOUT') {
            return new McpError(ErrorCode.RequestTimeout, `Operation timed out while running ${toolName}`);
        }
        if (error.code === 'EACCES') {
            return new McpError(ErrorCode.InvalidRequest, `Permission denied accessing ${toolName}`);
        }
        // Parse Verible-specific errors from stderr
        const veribleError = this.parseVeribleError(error.stderr || error.message, toolName);
        if (veribleError) {
            return new McpError(ErrorCode.InvalidRequest, veribleError.message, veribleError.details);
        }
        // Default error
        return new McpError(ErrorCode.InternalError, error.message || `Unknown error in ${toolName}`, { originalError: error.toString() });
    }
    /**
     * Parse Verible-specific error messages
     */
    static parseVeribleError(stderr, toolName) {
        if (!stderr)
            return null;
     ...
     */
    static async handleError(operation, toolName, fallback) {
        try {
