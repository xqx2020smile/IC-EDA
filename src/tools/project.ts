...
export class ProjectTool extends AbstractTool<ProjectParams, ProjectResult> {
  private tempFileListPaths: Map<string, string> = new Map();

  constructor(configManager: any, cacheManager: any, private indexManager: ProjectIndexManager) {
  ...
  private resolveFileListPath(params: ProjectParams): string | null {
    if (params.file_list) {
  ...
  private createFileListFromRoot(params: ProjectParams): string | null {
  ...
  private buildGlobConfig(rootPath: string): { cwd: string; pattern: string } | null {
    try {
  ...
  private buildTempFileKey(params: ProjectParams): string {
    const keyData = {
  ...
  private buildTempFilePath(key: string): string {
    const suffix = crypto.randomBytes(4).toString('hex');
    return path.join(os.tmpdir(), `verible-mcp-project-${key}-${suffix}.txt`);
  }

  private cleanupTempFile(params: ProjectParams): void {
    const key = this.buildTempFileKey(params);
